...
update Ticket set Estado='Cargado',Fecha_registro=getdate(),Archivos_exportados='no' where id=15
...
namespace Validador_Exportador
{
    public partial class Form1 : Form
    {
        ...
        public static string version = "may_2021";
        ...
        //LocalS
        //public string cadenaconexion = "Data Source= PE2313880W1\\SQLEXPRESS2014;Initial Catalog=Ticket_DgTAX; Integrated Security=True;";
        //Produccion
        public string rutaOrigen = @"J:\COMMON\";
        //Local
        //public string rutaOrigen = @"C:\Users\VS943JJ\OneDrive - EY\Desktop\CBC\";

        public string rutaDestino = @"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\";
        public Form1()
        {
                    ...
                    else
                    {
                        ...
                        if (textBox3.Text == usuario && Senior_ == usuario)
                        {
                            try
                            {
                                ...
                                path_Libros = rutaOrigen + textBox3.Text.Replace(".", "_");
                                ...
                                */
                                ...
                                if (File.Exists(path_Libros+ @"/FORMATO_PARA_IDENTIFICAR_L.E_VERSION_ACTUAL_y_ANTIGUA.xlsx"))
                                {
                                    #region Verificar que el contenido declarado exista en el ruta
                                    //Verificar el nombre de la Hoja

                                    if (!validarHoja(path_Libros + @"/FORMATO_PARA_IDENTIFICAR_L.E_VERSION_ACTUAL_y_ANTIGUA.xlsx", "Lista_de_archivos"))
                                    {
                                        button2.Enabled = false;
                                        flg_DatTribut = 0;
                                        return;
                                    ...
                                    var package = new ExcelPackage(new FileInfo(path_Libros+ @"/FORMATO_PARA_IDENTIFICAR_L.E_VERSION_ACTUAL_y_ANTIGUA.xlsx"));
                                    ...
                                    while (workSheet.Cells[i, 1].Value != null )
                                    {
                                        ...
                                        if (!File.Exists(path_Libros + @"/"+valor))
                                        {
                                            MessageBox.Show(" El archivo declarado en el formato excel no se encuentra en la ruta: "+valor,"Mensaje de ayuda" ,MessageBoxButtons.OK, MessageBoxIcon.Stop);
                                            button2.Enabled = false;
                                            flg_DatTribut = 0;
                                            return;
                                        }
                                        ...
                                        if(workSheet.Cells[i, 2].Value != null)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                MessageBox.Show("La fecha asociada al archivo: "+ valor + ", no es válida", "Mensaje de ayuda", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                                                button2.Enabled = false;
                                                flg_DatTribut = 0;
                                                return;
                                            }
                                        ...
                                        {
                                            MessageBox.Show(" El archivo : " + valor + " , no tiene una fecha de carga asociada", "Mensaje de ayuda", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                                            button2.Enabled = false;
                                            flg_DatTribut = 0;
                                            return;
                                        }
                                    ...
                                    //Listar los archivos que se encuentran en la ruta
                                    foreach (var file in di.GetFiles())
                                    {
                                        //Verificar si se encuentran en el formato Excel
                                        i = 2;
                                        Boolean flag_archivo = false;
                                        while (workSheet.Cells[i, 1].Value != null)
                                        {
                                            if(file.Name== workSheet.Cells[i, 1].Value.ToString() && file.Name!= "FORMATO_PARA_IDENTIFICAR_L.E_VERSION_ACTUAL_y_ANTIGUA.xlsx")
                                            {
                                                flag_archivo = true;                                                
                                            }
                                                i++;
                                        }
                                        if (!flag_archivo && file.Name != "FORMATO_PARA_IDENTIFICAR_L.E_VERSION_ACTUAL_y_ANTIGUA.xlsx")
                                        {
                                            //No se encontro el archivo en el excel
                                            MessageBox.Show(" El archivo en la ruta no se encuentra en el formato excel: " + file.Name, "Mensaje de ayuda", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                                            button2.Enabled = false;
                                            flg_DatTribut = 0;
                                            return;
                                        }
                                ...
                                foreach (var file in di.GetFiles()){
                                    if (file.Name.Length == 37 &&  file.Name.Substring(30, 1) == "2" && file.Extension.Contains("txt"))
                                    {
                                        MessageBox.Show("Las P.E no analizan los L.E en dólares ( LE________#2###.txt )" + "\r\n" + "Por favor retirar el txt: " +file.Name);
                                        button2.Enabled = false;
                                        return;
                                    }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        clearDirectory(rutaDestino + Cod_ticket_final);// Elimina la carpeta Destino por si ya existiera
                        ...
                        if (XML_Vent_Comp == "si")
                        {
                            ...
                            DirectoryInfo dirs = new DirectoryInfo(path_Libros);

                            foreach (var dir in dirs.GetDirectories())
                            {
                                if (dir.Name.ToLower().Contains("xml"))
                                {
                                    System.IO.Directory.Move(dir.FullName, rutaDestino + Cod_ticket_final + @"\" + dir.Name);
                                }
                            }
                        }
                        #endregion
        ...
        private void Mover_archivo_NOCVD(string ruta_origen, string archivo)
        {
            try
            {
                ...
                destinationFile = rutaDestino + Cod_ticket_final + "\\" + archivo;
                ...
                if (!Directory.Exists(@"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FormatoNuevo\"))
                {
                    Directory.CreateDirectory(@"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FormatoNuevo\");
                }

                if (!Directory.Exists(@"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FormatoAntiguo\"))
                {
                    Directory.CreateDirectory(@"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FormatoAntiguo\");
                }
                ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
                var package = new ExcelPackage(new FileInfo(path_Libros + @"/FORMATO_PARA_IDENTIFICAR_L.E_VERSION_ACTUAL_y_ANTIGUA.xlsx"));
                ...
                while (workSheet.Cells[i, 1].Value != null)
                {
                    ...
                    if (valor == archivo)
                    {
                        ...
                        {
                            //Formato nuevo
                            if (File.Exists(@"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FormatoNuevo\" + valor))
                            {
                                File.Delete(@"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FormatoNuevo\" + valor);
                            }
                            System.IO.File.Copy(ruta_origen, @"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FormatoNuevo\" + valor);

                        }
                        else
                        {
                            //Formato antiguo
                            if (File.Exists(@"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FormatoAntiguo\" + valor))
                            {
                                File.Delete(@"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FormatoAntiguo\" + valor);
                            }
                            System.IO.File.Copy(ruta_origen, @"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FormatoAntiguo\" + valor);
                        }
                        break;
                    }
                    i++;
                ...
                if (!File.Exists(@"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FORMATO_PARA_IDENTIFICAR_L.E_VERSION_ACTUAL_y_ANTIGUA.xlsx"))
                {
                    System.IO.File.Copy(path_Libros + @"\FORMATO_PARA_IDENTIFICAR_L.E_VERSION_ACTUAL_y_ANTIGUA.xlsx", @"J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB\" + Cod_ticket_final + @"\FORMATO_PARA_IDENTIFICAR_L.E_VERSION_ACTUAL_y_ANTIGUA.xlsx");
                }
            }
            catch (Exception ex)
            {
        ...
        private void Mover_archivo_CVD_1(string ruta_origen, string archivo)
        {
            try
            {
                ...
                //J:\COMMON\Eddy Fernando TAX\NO_PROCESADO_SIZE_100MB
                string destinationFile = rutaDestino + Cod_ticket_final;
                #region En caso se tenga un archivo con el mismo nombre se elimina para poder mover el archivo traido por parametro
                if (!Directory.Exists(destinationFile))
                    Directory.CreateDirectory(destinationFile);
        ...
        private void ValidarVersion() {
            ...
            SqlCommand com = new SqlCommand("select top 1(descripcion) from validador_m_version order by id desc;", conn);
            conn.Open();

            string newVersion = (string)com.ExecuteScalar();
            conn.Close();

            if (version != newVersion)
            {
                MessageBox.Show("Porfavor descarge la última Actualización '" + newVersion + "',\nGracias!", "Versión antigua! :"+version, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                this.Close();
            }
            //else { MessageBox.Show("Bienvenido!!!"); }//Esto es solo para comprobar de que funciona, luego se borrara esta linea
        }

        private void clearDirectory(string path_ticket) {

            if (Directory.Exists(path_ticket)) {
                Directory.Delete(path_ticket,true);
            }
        }
    }
}
